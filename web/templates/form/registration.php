...
<form method="post" enctype="multipart/form-data">
    ...
    <input name="first-name" required pattern="[a-zA-ZА-Яа-яЁё\p]+" value="<?=isset($data['firstName'])?$data['firstName']:false?>"><br>
    <label for="last-name" style="padding-right: 10pt">Фамилия: </label>
    <input name="last-name" required pattern="[a-zA-ZА-Яа-яЁё\p]+" value="<?=isset($data['lastName'])?$data['lastName']:false?>"><br>
    <label for="nickname" style="padding-right: 10pt">Никнейм: </label>
    <input name="nickname" required pattern="\w{4,10}"  oninvalid="setCustomValidity('minimum 4 and max 10 character')"
    ...
    <input type="password" name="password1" required pattern="\w{4,10}"  oninvalid="setCustomValidity('minimum 4 and max 10 character')"
           onchange="try{setCustomValidity('')}catch(e){}"><br>
    <label for="nickname">Повторите: </label>
    <input type="password" name="password2" required pattern="\w{4,10}"  oninvalid="setCustomValidity('minimum 4 and max 10 character')"
           onchange="try{setCustomValidity('')}catch(e){}"><br>
    <label for="email" style="padding-right: 31pt">e-mail: </label>
    <input type="email" name="email" required value="<?=isset($data['email'])?$data['email']:false?>"><br>
    <label for="country" style="padding-right: 20pt">Страна: </label>
    <input name="country" required pattern="[a-zA-ZА-Яа-яЁё\p]+" value="<?=isset($data['country'])?$data['country']:false?>"><br>
